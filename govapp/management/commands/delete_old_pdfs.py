...
from govapp.models import PdfDetail  # apne model ka naam use karo
import os
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        cutoff_date = timezone.now() - timezone.timedelta(seconds=5)  # abhi ke liye 5 sec
        old_pdfs = PdfDetail.objects.filter(created_at__lt=cutoff_date, pdf__isnull=False).exclude(pdf='')
